...
class UserManagerPDO extends UserManager
{
	...
	public function getUnique($id /*getIdFromLogin($_SESSION['login']*/)
    {
        if (isset($id) && !empty($id)) {
            $DB_REQ = $this->DB_REQ->prepare('
                SELECT id, login, email, firstName, lastName, age, gender, password, hash, sexuality, bio, isactive
                FROM users
                WHERE id = :id
                ');
    ...
    {
        $tagsArray = array('algorythm', 'graphics', 'unix', 'sysadmin', 'web');
        if (isset($tags))
        {
            $values = array();
            foreach ($tags as $choice)
