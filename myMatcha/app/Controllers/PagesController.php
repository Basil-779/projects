...
class PagesController extends Controller 
{
	...
	function getNotifs() {
		$notificationManager = new NotificationManager($this->db);
		$notifs = $notificationManager->getNotifs($_SESSION['id']);
		$UserManagerPDO = new UserManagerPDO($this->db);
		foreach ($notifs as &$value) {
			$value['id_belong'] = $UserManagerPDO->getLoginFromId($value['id_belong']);
			$value['login'] = $UserManagerPDO->getLoginFromId($value['id_sender']);
		}
		return $notifs;
	}
	...
	function readNotif($notif_id) {
		$notificationManager = new NotificationManager($this->db);
		$notificationManager->setNotifAsRead($_SESSION['id'], $notif_id);
		return "OK";
	}
	
	function setOnlineTime() {
		$UserManagerPDO = new UserManagerPDO($this->db);
		$UserManagerPDO->updateLastSeen($_SESSION['id']);
	}
	
	/*********************************** END TEST  *********************************/
	
	
	public function getNotFound($request, $response) {
		$this->render($response, '/404.twig', ['login' => $_SESSION['login']]);
	}
	
	public function postNotFound($request, $response) {
		if ($request->getParam('listen')) {
			return json_encode($this->getNotifs());
		}
		if ($request->getParam('read')) {
			return $this->readNotif($request->getParam('read'));
		}
		if ($request->getParam('online')) {
			$this->setOnlineTime();
		}
	}
		...
		if (Validator::isConnected()) {
			if (Validator::isComplete($_SESSION['login'], $this->container->db)) {
				...
				foreach ($matches as $key => $value)
				{
					$tmp = $UserManagerPDO->getUnique($key);
					$tmp['distance'] = $value;
					$tmp['online'] = $UserManagerPDO->isOnline($key);		
					$users[] =  $tmp;
				}
	...
	public function postMain($request, $response)
    {
		...
		switch ($action) {
			case "filter":
				...
				$tagsInCommon = (int)$request->getParam('t_max');
				$matches = $UserManagerPDO->getMatches($id, $gender, $sexuality, $sort, $ageMin, $ageMax, $scoreMin, $scoreMax, $locationMax, $tagsInCommon);
				foreach ($matches as $key => $value)
				{
					$tmp = $UserManagerPDO->getUnique($key);
	...
	public function getChatlist($request, $response)
    {
		...
		if (empty($errors))
		{
			...
			$this->render($response, '/chatlist.twig', ['chatlist' => json_encode($chatUsers), 'login' => $_SESSION['login'], 'count' => $this->countNotif($id)]);
		}
		/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
	...
	public function getChat($request, $response, $args)
    {
		...
		if (Validator::isConnected())
		{
			if (Validator::isComplete($_SESSION['login'], $this->container->db))
			{
				...
				{
					if ($this->hasLiked((int)$_SESSION['id'], (int)$args['chat_id']) && $this->hasLiked((int)$args['chat_id'], (int)$_SESSION['id']))
					{
						$UserManagerPDO = new UserManagerPDO($this->db);
						$profile = [];
						$messages = $UserManagerPDO->getChatMsg($_SESSION['id'], $args['chat_id']);
						foreach ($messages as &$value) {
							$value['id_poster'] = $UserManagerPDO->getName($value['id_poster']);
						}
						$name = $UserManagerPDO->getName($args['chat_id']);
						$self_name = $UserManagerPDO->getName($_SESSION['id']);
						$online = $UserManagerPDO->isOnline($args['chat_id']);
						$profile['name'] = $name;
						$profile['self_name'] = $self_name;
						$profile['online'] = $online;
						$this->render($response, '/chat.twig', ['chat_messages' => json_encode($messages), 'profile' => json_encode($profile), 'login' => $_SESSION['login'], 'count' => $this->countNotif($id)]);
					}
					else {
						$error = "Sorry, this user unliked you.";
						$this->render($response, '/404.twig', ['error' => $error, 'login' => $_SESSION['login']]);
					}
				}
				else {
					$error = "No such user.";
				...
				$error = "You must fullfill your account before chatting";
			}
		}
		else {
	...
	public function postChat($request, $response, $args)
    {
		...
		if ($args['chat_id']) {
			...
			if (!empty($request->getParam('message')))
			{
				...
				$notificationManager = new NotificationManager($this->db);
				$notificationManager->add($notification);
				/*$messages = $UserManagerPDO->getNewChatMsgNoUpdate($_SESSION['id'], $login);
				foreach ($messages as &$value) {
					$value['id_poster'] = $UserManagerPDO->getName($value['id_poster']);
				}
		...
		if ($request->getParam('new')) {
			$messages = $UserManagerPDO->getUnreadMsg($_SESSION['id'], $request->getParam('new'));
			foreach ($messages as &$value) {
				$value['id_poster'] = $UserManagerPDO->getName($value['id_poster']);
			}
			return json_encode($messages);
	...
	public function getChangePassword($request, $response, $args)
    {
		...
		$UserManagerPDO = new UserManagerPDO($this->db);
		if ($UserManagerPDO->searchHash($_SESSION['hash'])) {
			
			return $this->render($response, '/accountNewpass.twig');
		}
		else {
			$error = "Bad link";
			$this->render($response, '/404.twig', ['error' => $error]);
		}
    }
    ...
    public function postSignUpInfos($request, $response)
    {
		...
		if ($request->getParam('action') === 'delete') {
			$userManagerPDO = new UserManagerPDO($this->db);
			$userManagerPDO->deletePicture($request->getParam('src'), $_SESSION['id']);
			return "OK";
		}
		if ($request->getParam('action') === 'setmain') {
			$userManagerPDO = new UserManagerPDO($this->db);
			$userManagerPDO->setMainPicture($request->getParam('src'), $_SESSION['id']);
			return "OK";
		}
        ...
        if (empty($errors))
        {
            ...
            {
                $errors_photo = $this->postUploadPicture($request, $response);
            }
			if (empty($errors_photo)) {
				return "OK";
			}
			else {
				return json_encode($errors_photo);
			}
        }
        else
        {
