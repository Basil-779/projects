...
function sortTags($id, $ids)
{
    $res1 = [];
    foreach ($ids as $id_to_comp)
    {
        $res1[] = compareTags($id, $id_to_comp);
    }
    $res = array_combine($ids, $res1);
    arsort($res);
    return array_keys($res);
}

function compareTags($id1, $id2)
{
    $tagsArray1 = get_tags($id1);
    $tagsArray2 = get_tags($id2);
    $i = 0;
    $similar = 0;
    while ($i < 5)
    {
        if (($tagsArray1[$i] == $tagsArray2[$i]) && ($tagsArray1[$i] == 1) && ($tagsArray2[$i] == 1))
        {
            $similar++;
        }
        $i++;
...
$DB_REQ->bindValue(':scoreMax', $scoreMax, PDO::PARAM_INT);
$DB_REQ->execute();
$ids1 = $DB_REQ->fetchAll(PDO::FETCH_NUM);
$sexualityS = 'bisexual';
$DB_REQ = new PDO('mysql:host=localhost;dbname=rootpokrov_matcha', 'rootpokrov', 'yh3zuey@fGhu');
$DB_REQ = $DB_REQ->prepare('
...
$DB_REQ->bindValue(':gender', $genderS, PDO::PARAM_STR);
$DB_REQ->bindValue(':id', $id, PDO::PARAM_INT);
$DB_REQ->bindValue(':ageMin', $ageMin, PDO::PARAM_INT);
$DB_REQ->bindValue(':ageMax', $ageMax, PDO::PARAM_INT);
$DB_REQ->bindValue(':scoreMin', $scoreMin, PDO::PARAM_INT);
