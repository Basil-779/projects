...
class UserManagerPDO extends UserManager
{
	...
	public function setMainPicture($source, $UserId) {
        if ($source){
            $DB_REQ = $this->DB_REQ->prepare('
                UPDATE pictures
                SET mainpic = :value
	...
	public function saveMsg($poster, $receptor, $message) {
        $DB_REQ = $this->DB_REQ->prepare('
            INSERT INTO chat (id_poster, id_receptor, message, date_message, unread)
            VALUES (:id_poster, :id_receptor, :message, NOW(), 0)
            ');
	...
	public function getUnreadMsg($user1, $user2)
    {
        $DB_REQ = $this->DB_REQ->prepare('
            SELECT *
            FROM chat
            WHERE (id_poster = :user2 AND id_receptor = :user1 AND unread = 0)
            ORDER BY date_message;
            UPDATE chat
            SET unread = 1 WHERE (id_poster = :user2 AND id_receptor = :user1 AND unread = 0);
        ');
        $DB_REQ->bindValue(':user1', $user1);
        $DB_REQ->bindValue(':user2', $user2);
        $DB_REQ->execute();
	...
	public function getNewChatMsg($user1, $user2) {
        $DB_REQ = $this->DB_REQ->prepare('
            SELECT *
            FROM chat
            WHERE (id_poster = :user1 AND id_receptor = :user2 AND unread = 0)
            OR (id_poster = :user2 AND id_receptor = :user1 AND unread = 0)
            ORDER BY date_message;
            UPDATE chat SET unread = 1;
        ');
        $DB_REQ->bindValue(':user1', $user1);
        $DB_REQ->bindValue(':user2', $user2);
        $DB_REQ->execute();
	...
	public function getNewChatMsgNoUpdate($user1, $user2) {
        $DB_REQ = $this->DB_REQ->prepare('
            SELECT *
            FROM chat
            WHERE (id_poster = :user1 AND id_receptor = :user2 AND unread = 0)
	...
	public function getName($user_id) {
		$DB_REQ = $this->DB_REQ->prepare('
			SELECT CONCAT(firstName, " ", lastName) AS name FROM users WHERE id = :id
		');
	
		$DB_REQ->bindValue(':id', $user_id, PDO::PARAM_INT);
		$DB_REQ->execute();
		$data = $DB_REQ->fetchColumn();
        return $data;
    }
	public function searchHash($hash)
	{
		$DB_REQ = $this->DB_REQ->prepare('
		SELECT count(*) FROM users WHERE hash = :hash
		');
		$DB_REQ->bindValue(':hash', $hash);
		$DB_REQ->execute();
		$data = $DB_REQ->fetch();
